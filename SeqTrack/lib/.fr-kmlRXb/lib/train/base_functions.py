...
# datasets related
from lib.train.dataset import Lasot, Got10k, MSCOCOSeq, ImagenetVID, TrackingNet, Imagenet1k, Whispers
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "COCO17", "VID", "TRACKINGNET", "IMAGENET1K", 'WHISPERS']
        if name == "LASOT":
        ...
        if name == 'WHISPERS':
            datasets.append(Whispers(settings.env.whispers, 'HSI-RedNIR'))
    return datasets
