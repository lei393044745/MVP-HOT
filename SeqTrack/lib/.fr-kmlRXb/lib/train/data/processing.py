...
class SeqTrackProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            data['template_images'], data['template_anno'], data['template_masks'] = self.transform['joint'](
                image=data['template_images'], bbox=data['template_anno'], mask=data['template_masks'])
            data['template_hsi_images'], _, _ = self.transform['joint'](
                image=data['template_hsi_images'], bbox=data['template_anno'], mask=data['template_masks'])
            data['search_images'], data['search_anno'], data['search_masks'] = self.transform['joint'](
                image=data['search_images'], bbox=data['search_anno'], mask=data['search_masks'], new_roll=False)
            data['search_hsi_images'], _, _ = self.transform['joint'](
                image=data['search_hsi_images'], bbox=data['search_anno'], mask=data['search_masks'], new_roll=False)
        ...
        for s in s_list:
            ...
            crops_hsi, _, _, _ = prutils.jittered_center_crop(data[s + '_hsi' + '_images'], jittered_anno,
                                                                              data[s + '_anno'], self.search_area_factor[s],
                                                                              self.output_sz[s], masks=data[s + '_masks'])
            ...
            data[s + '_hsi' + '_images'], _, _ = self.transform[s](
                image=crops_hsi, bbox=boxes, mask=mask_crops, joint=False)
