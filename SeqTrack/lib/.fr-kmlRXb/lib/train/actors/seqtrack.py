...
class SeqTrackActor(BaseActor):
    ...
    def forward_pass(self, data):
        ...
        search_hsi_img = data['search_hsi_images'].view(-1, *data['search_hsi_images'].shape[2:])
        search_hsi_list = search_hsi_img.split(b,dim=0)
        template_img = data['template_images'].view(-1, *data['template_images'].shape[2:])
        template_list = template_img.split(b,dim=0)
        template_hsi_img = data['template_hsi_images'].view(-1, *data['template_hsi_images'].shape[2:])
        template_hsi_list = template_hsi_img.split(b,dim=0)
        feature_xz = self.net(images_list=template_list+search_list, images_hsi_list =template_hsi_list + search_hsi_list,mode='encoder') # forward the encoder
