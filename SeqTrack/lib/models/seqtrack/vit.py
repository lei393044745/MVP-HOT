...
from .promptv2 import Prompt_all
import numpy as np
...
class Hsi_PatchEmbed(nn.Module):
    def __init__(self, in_dim = None):
        super().__init__()
        self.hsi_patch_embed = nn.Sequential(
            nn.Conv2d(in_channels=in_dim, out_channels=in_dim, kernel_size=(16, 16), stride=16, groups=in_dim),
            nn.Conv2d(in_dim, 768, 1, 1)
        )
    ...
    def forward(self, x):
        return self.hsi_patch_embed(x).flatten(2).transpose(1, 2)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, search_size=384, template_size=192,
        ...
        #         img_size=search_size, patch_size=patch_size, in_chans=16, embed_dim=embed_dim)
        self.hsi_patch_embed = Hsi_PatchEmbed(in_dim =16)
        self.prompt_depth = 1
        self.prompt = Prompt_all(self.prompt_depth)
        self.lamdas = np.zeros(self.prompt_depth)
        # self.lamdas2 = np.zeros(depth)
        # self.lamdas = nn.Parameter(torch.zeros(depth))
        ...
        self.hsi_pos_embed = nn.Parameter(torch.zeros(1, self.num_patches_search + self.num_patches_template, embed_dim))
        ...
        trunc_normal_(self.hsi_pos_embed, std=.02)
    ...
    def forward_features(self, images_list, images_hsi_list):
        num_template = self.num_template
        template_list = images_list[0:num_template]
        search_list = images_list[num_template:]
        
        template_hsi_list = images_hsi_list[0:num_template]
        search_hsi_list = images_hsi_list[num_template:]
        ...
        z_hsi_list = []
        for i in range(num_template):
            z = template_hsi_list[i]
            z = self.hsi_patch_embed(z)
            # .flatten(2).transpose(1, 2)
            z = z + self.hsi_pos_embed[:, self.num_patches_search:, :]
            z_hsi_list.append(z)
        z_hsi_feat = torch.cat(z_hsi_list, dim=1)
        ...
        for i in range(num_search):
            x = search_hsi_list[i]
            x = self.hsi_patch_embed(x)
            # .flatten(2).transpose(1, 2)
            x = x + self.hsi_pos_embed[:, :self.num_patches_search, :]
            x_hsi_list.append(x)
        x_hsi_feat = torch.cat(x_hsi_list, dim=1)
        
        xz_feat = torch.cat([x_feat, z_feat], dim=1)
        xz_hsi_feat = torch.cat([x_hsi_feat, z_hsi_feat], dim = 1)
        ...
        for blk in self.blocks:   #batch is the first dimension.
            #norm
            # out = self.prompt(xz, out, idx)
            # xz = xz + out
            #fixed
            if idx < self.prompt_depth:
                out = out + self.prompt(xz, out, idx)    
                xz = xz + min(self.lamdas[idx], 1) * out
                self.lamdas[idx] += 0.0005
                
            #norm & fixed
            # out = out + self.prompt(xz, out, idx)
            # xz = xz + min(self.lamdas[idx], 1) *  out
            # self.lamdas[idx] += 0.0001
    ...
    def forward(self, images_list, images_hsi_list):
        xz = self.forward_features(images_list, images_hsi_list)
        out=[xz]
        return out
