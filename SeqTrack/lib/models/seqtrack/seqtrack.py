...
class SEQTRACK(nn.Module):
    ...
    def forward(self, images_list=None, images_hsi_list=None , xz=None, seq=None, mode="encoder"):
        """
        ...
        """
        if mode == "encoder":
            return self.forward_encoder(images_list, images_hsi_list)
        elif mode == "decoder":
            return self.forward_decoder(xz, seq)
        else:
            raise ValueError

    def forward_encoder(self, images_list, images_hsi_list):
        # Forward the encoder
        xz = self.encoder(images_list, images_hsi_list)
        return xz
