...
def eval():
    ...
    tracker_names = ['MVP-HOT','BAE-Net','MHT','SEE-Net', 'SeqTrack', 'HA-Net', 'SiamHYPER']
    # s_types = ['base', 'prompt']
    performance = {}
    report_dir = './report2/'
    report_file = os.path.join(report_dir, 'performance.json')
    ...
    for name in tracker_names:
        ...
        for s, res_path in enumerate(all_list):
            ...
            path = gt_path + '/' + t[:-4] + '/HSI'
            
            gt = np.loadtxt(path + '/groundtruth_rect.txt')
            type_arr = []
